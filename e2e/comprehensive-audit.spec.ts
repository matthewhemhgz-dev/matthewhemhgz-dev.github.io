    ...
    test.describe(`页面: ${pageInfo.name}`, () => {
      test('检查浅色模式可访问性', async ({ page }) => {
        ...
        await page.addStyleTag({
          content:
            '.noise-overlay, #particles-canvas, .reveal, [data-reveal] { display: none !important; opacity: 1 !important; visibility: visible !important; }',
        });
        ...
        console.log(
          `Found ${accessibilityScanResults.violations.length} accessibility violations for ${pageInfo.name}`,
        );
      });
      ...
      for (const viewport of VIEWPORTS) {
        test(`响应式检查: ${viewport.name} (${viewport.width}x${viewport.height})`, async ({
          page,
        }) => {
          ...
          if (hasHorizontalScroll) {
            console.warn(
              `[WARNING] Horizontal scroll detected on ${pageInfo.name} at ${viewport.name}`,
            );
          }
